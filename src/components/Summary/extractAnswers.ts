export default function extractAnswers(answers: any, survey: any, config: any) {
	...
	questions.forEach((question: any) => {
		if (
			...
			// No answer
		} else if (question.type == 'matrix' || question.type == 'matrixdropdown') {
			...
			// Image (TODO: Consider adding more testing)
			...
			// Questions with choices (radio, checkbox, etc)
			if (answers[question.name] == 'other') {
				// Other and none does not show up as normal answers
			...
			} else if (Array.isArray(answers[question.name])) {
				// Multiple answers
				newAnswers[question.name] = answers[question.name]
					.map((answer: any) => {
			...
			if (question.inputType == 'date') {
				newAnswers[question.name] = new Date(answers[question.name]).toLocaleDateString(
					config?.locale,
					{ year: 'numeric', month: '2-digit', day: '2-digit' }
				);
			} else if (question.inputType == 'datetime-local') {
				newAnswers[question.name] = new Date(answers[question.name]).toLocaleTimeString(
					config?.locale,
					{
						year: 'numeric',
						month: '2-digit',
				...
				newAnswers[question.name] =
					(config?.locale === 'no' ? 'Uke ' : 'Week ') +
					answers[question.name].split('-W')[1] +
					', ' +
					answers[question.name].split('-W')[0];
			} else {
				newAnswers[question.name] = answers[question.name];
			}
